...
1. Fungsi power() dipanggil di dalam term() karena dalam hierarki operator (Operator Precedence), 
   pangkat memiliki prioritas lebih tinggi daripada perkalian/pembagian. 
   Fungsi yang dipanggil lebih dalam akan dieksekusi lebih dulu dalam AST.

2. Jika variabel 'z' tidak ada di symbol_table, akan terjadi Semantic Error. 
   Compiler akan melempar ParserError "Undefined variable" karena tidak dapat menemukan nilai untuk variabel tersebut.

3. Dalam TAC, instruksi 'a ^ 2' harus muncul sebelum '+' karena hasil dari pangkat 
   dibutuhkan sebagai input untuk operasi penjumlahan berikutnya. TAC mengikuti urutan eksekusi logis.
"""
